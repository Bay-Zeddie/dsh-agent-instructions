...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CLIENT_BUILD = 'ui-2026-09-21z4'
    const POLL_MS = 10000
     ...
     * 当前语言。
     *
     * ⚠️ **必须每次现读**，不能在初始化时抓一次 —— dsh 是启动之后才把
     * `<html lang>` 写上的，早期读到的是空值/默认英文（实测踩过：抓到 'en'，
     * 结果设置页导航项被注册成 "Agent settings"）。
     * 优先 `<html lang>`，为空时回落 `navigator.language`。
     */
    function currentLang() {
      try {
        const fromDoc = String(document.documentElement.lang || '').trim()
        if (fromDoc) return fromDoc.toLowerCase().startsWith('en') ? 'en' : 'zh'
        const fromNav = String(navigator.language || '').trim()
        if (fromNav) return fromNav.toLowerCase().startsWith('en') ? 'en' : 'zh'
        return 'zh'
      } catch {
        return 'zh'
      }
    ...
    /** 带护栏头的 fetch：Host 侧要求 `x-dsh-agent: 1`。 */
    function apiFetch(path, options) {
      const init = options || {}
      const headers = Object.assign({ [GUARD_HEADER]: '1' }, init.headers || {})
...
`
    ...
    function mountSettingsEntry(ctx) {
      if (!ctx || !ctx.slots || typeof ctx.slots.inject !== 'function') {
        console.info('[dsh-agent] 环境未提供 slots 服务，跳过设置页入口')
        return false
      }
      ...
      const landed = () => {
        try {
          ...
          console.info('[dsh-agent] settings.section ledger = ' + JSON.stringify(ids))
          return ids.indexOf(NS) !== -1
        } catch (error) {
          console.info('[dsh-agent] ledger 自省不可用：' + ((error && error.message) || error))
          return true
        }
      }
      ...
      setTimeout(() => {
        if (landed()) {
          console.info('[dsh-agent] 设置页独立页已就绪（settings.section）')
          return
        }
    ...
    function registerLocale(ctx) {
      try {
        ...
        console.info('[dsh-agent] locale 注册跳过：', error && error.message)
        return false
      }
    }
    ...
    function apply(ctx) {
      ...
      // 语言可能在启动后才被 dsh 写入 `<html lang>`：变化时刷新浮动按钮文案，
      // 并按官方契约用新文案重新注册设置页入口。
      let stopWatch = () => {}
      try {
        stopWatch = watchLang(() => {
