 ...
 * 客户端声明的界面语言。
 *
 * ⚠️ **Host 侧不要写死面向用户的文案。** 这些字符串是当作**数据**发给客户端原样渲染的，
 * 不经过任何翻译层，所以写死中文就会在英文界面里露出来（本插件曾如此：`planeNote` 那行
 * 是整块英文面板里唯一的中文）。客户端每个请求都会带上这个头，Host 据此挑文案 ——
 * 而客户端自己又是跟随 dsh 的 `locale` 服务取值，于是**语言自动跟随 dsh**。
 */
const LANG_HEADER = 'x-dsh-lang'

/** 把任意 BCP 47 标签归一成插件支持的 `zh` / `en`；空值返回空串（表示"没给信号"）。 */
 ...
 * 本次请求该说哪种语言。
 *
 * ① 客户端显式声明的 `x-dsh-lang`（权威，因为它是从 dsh 的 locale 服务取的）
 * ② `accept-language`（客户端缺席时的近似）
 * ③ 回落 `zh` —— **保持既有默认行为**，老的调用方（如通用设置行）不退化。
 */
function requestLang(req) {
  try {
    const own = normalizeLang(req?.headers?.[LANG_HEADER])
    if (own) return own
    const accept = normalizeLang(req?.headers?.['accept-language'])
...
/**
 * Host 侧文案表。取值可以是字符串，也可以是**接收占位参数的函数**。
 *
 * 与客户端 `MESSAGES` 同约定：两边键名同义、缺失回落中文，
 * 因此永远不会渲染出 `undefined` 或空串。
 * 新增文案时 zh/en **必须同时补齐**（`tests/audit-bundle.mjs` 做双向核对）。
 */
const TEXT = {
  zh: {
    ctMustBeJson: 'content-type 必须是 application/json',
    methodNotAllowed: '该路由不接受这个请求方法（只读了 GET，写入只收 PUT/POST）',
    bodyTooLarge: (limit) => `请求内容超过 ${limit} 字节上限`,
    bodyNotJson: '请求体不是合法 JSON',
    bodyReadFailed: '请求体读取失败',
    badCwd: 'cwd 非法',
    missingTarget: '缺少 target 路径',
    targetOutOfChain: '拒绝：该路径不在当前指令链内（只允许官方候选名所在的位置）',
    missingContent: '缺少 content 字段',
    contentTooLarge: (max) => `内容超过官方单文件上限 ${max} 字节 —— 超过该值的文件会被官方静默忽略`,
    conflict: 'conflict: 磁盘上的文件已被改动，未保存以免覆盖',
    missingGlobalEnabled: '缺少 globalEnabled 布尔字段',
    noGlobalToPause: (path) => `找不到 ${path}，没有可暂停的全局文件`,
    globalEnabled: (path) => `全局 AGENTS.md 已启用：${path}`,
    globalPaused: (path) => `全局 AGENTS.md 已暂停（文件保留）：${path}`,
    switchFailed: (msg) => `切换失败：${msg}`,
    badMode: 'mode 必须是 both | project | global',
    missingModeOrChanges: '缺少 mode 或 changes 字段',
    tooManyLayers: '一次最多修改 64 层',
    badChangeItem: 'changes 每一项都需要 base(字符串) + enabled(布尔)',
    layerOutOfChain: (base) => `拒绝：${base} 不是当前指令链内的候选位置`,
    unknown: 'unknown',
    planePresetNotFound: '未能定位 preset 声明，按官方默认上限估算',
    planeNoDefaultPreset: 'settings.yaml 未记录默认 preset，按官方默认上限估算',
    planePresetNoRow: (presetId) => `preset「${presetId}」未声明 agent-instructions ⇒ 该 preset 下 AGENTS.md 完全不加载`,
    planePresetCap: (presetId, maxBytes, source) =>
      `preset「${presetId}」声明上限 ${maxBytes} 字节（${source === 'user-preset' ? '用户 preset' : '官方 preset'}，只读）`,
  },
  en: {
    ctMustBeJson: 'content-type must be application/json',
    methodNotAllowed: 'This route does not accept that method (reads are GET; writes are PUT/POST)',
    bodyTooLarge: (limit) => `Request body exceeds the ${limit} byte limit`,
    bodyNotJson: 'Request body is not valid JSON',
    bodyReadFailed: 'Failed to read the request body',
    badCwd: 'Invalid cwd',
    missingTarget: 'Missing target path',
    targetOutOfChain: 'Refused: that path is not in the current instruction chain (only official candidate locations are allowed)',
    missingContent: 'Missing content field',
    contentTooLarge: (max) => `Content exceeds the official per-file limit of ${max} bytes — files above it are silently ignored by the harness`,
    conflict: 'conflict: the file on disk changed; not saved, to avoid overwriting it',
    missingGlobalEnabled: 'Missing boolean field globalEnabled',
    noGlobalToPause: (path) => `Cannot find ${path} — there is no global file to pause`,
    globalEnabled: (path) => `Global AGENTS.md enabled: ${path}`,
    globalPaused: (path) => `Global AGENTS.md paused (file kept): ${path}`,
    switchFailed: (msg) => `Switch failed: ${msg}`,
    badMode: 'mode must be one of both | project | global',
    missingModeOrChanges: 'Missing mode or changes field',
    tooManyLayers: 'At most 64 layers can be changed at once',
    badChangeItem: 'Each changes entry needs base(string) + enabled(boolean)',
    layerOutOfChain: (base) => `Refused: ${base} is not a candidate location in the current instruction chain`,
    unknown: 'unknown',
    planePresetNotFound: 'Could not locate the preset declaration; estimated with the official default limit',
    planeNoDefaultPreset: 'settings.yaml records no default preset; estimated with the official default limit',
    planePresetNoRow: (presetId) => `preset "${presetId}" declares no agent-instructions ⇒ AGENTS.md is not loaded at all under that preset`,
    planePresetCap: (presetId, maxBytes, source) =>
      `preset "${presetId}" declares a ${maxBytes} byte limit (${source === 'user-preset' ? 'user preset' : 'official preset'}, read-only)`,
  },
}
 ...
 * 取文案。
 *
 * ⚠️ **参数是可变的** —— 带占位的文案按顺序多传：`T(lang, 'bodyTooLarge', limit)`。
 * 与客户端 `t()` 同一约定，别只接收一个参数。
 *
 * @param {string} lang 由 `requestLang(req)` 得到
 * @param {string} key 与 `TEXT` 键名对应
 */
function T(lang, key) {
  const args = Array.prototype.slice.call(arguments, 2)
  const dict = TEXT[lang] || TEXT.zh
  const value = dict[key] !== undefined ? dict[key] : TEXT.zh[key]
...
function guard(req, res, write) {
  ...
  if (write) {
    ...
    if (!contentType.startsWith('application/json')) {
      writeJson(res, 415, { error: T(requestLang(req), 'ctMustBeJson') })
      return false
    }
  }
  return true
...
function readJsonBody(req, limit) {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    req.on('data', (chunk) => {
      size += chunk.length
      if (size > limit) {
        const error = new Error(T(requestLang(req), 'bodyTooLarge', limit))
    ...
    req.on('end', () => {
      ...
      try {
        ...
        const error = new Error(T(requestLang(req), 'bodyNotJson'))
 ...
 * 拒绝盘符根，避免把整个盘当成工作区。
 */
function resolveWorkspaceDir(rawCwd, lang) {
  if (typeof rawCwd !== 'string' || rawCwd.trim() === '') {
    return { dir: process.cwd() }
  }
  const dir = resolve(rawCwd.trim())
  if (!dir || dir === resolve(dir, sep)) return { error: T(lang, 'badCwd') }
  return { dir }
}
 ...
 */
async function resolveTargetPath(rawTarget, workspaceDir, dshHome, lang) {
  if (typeof rawTarget !== 'string' || rawTarget.trim() === '') return { error: T(lang, 'missingTarget') }
  ...
  if (!hit) return { error: T(lang, 'targetOutOfChain') }
 ...
 */
async function resolveInstructionConfig(lang) {
  ...
  const fallback = {
    ...
    presetPath: null,
    planeNote: T(lang, 'planePresetNotFound'),
  }
  if (!presetId) return { ...fallback, planeNote: T(lang, 'planeNoDefaultPreset') }
  ...
  for (const root of roots) {
    ...
    if (!parsed.enabled) {
      return {
        ...
        presetPath,
        planeNote: T(lang, 'planePresetNoRow', presetId),
      }
    }
    return {
      presetId,
      ...
      instructionsEnabled: true,
      presetPath,
      planeNote: T(lang, 'planePresetCap', presetId, parsed.maxBytes ?? DEFAULT_MAX_BYTES, root.source),
    }
  }
  return fallback
}
 ...
 */
async function buildBudget(rows, lang) {
  const instruction = await resolveInstructionConfig(lang)
  const globalRow = rows.filter((row) => row.layer === 'user-global')[0]
  return {
    ...
    const disposers = [
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const workspace = resolveWorkspaceDir(rawCwd, lang)
          if (workspace.error) {
          ...
          try {
            ...
            if (typeof rawTarget === 'string' && rawTarget.trim() !== '') {
              const resolved = await resolveTargetPath(rawTarget, workspace.dir, global.dshHome, lang)
              if (resolved.error) {
            ...
            const budget = await buildBudget(layersView, lang)
            writeJson(res, 200, {
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (req.method !== 'PUT' && req.method !== 'POST') {
            if (!guard(req, res, false)) return
            writeJson(res, 405, { error: T(lang, 'methodNotAllowed') })
            return
          }
          ...
          try {
            ...
            writeJson(res, tooLarge ? 413 : 400, { error: error?.message ?? T(lang, 'bodyReadFailed') })
            return
          }
          const scope = payload.scope === 'global' ? 'global' : 'workspace'
          if (typeof payload.content !== 'string') {
            writeJson(res, 400, { error: T(lang, 'missingContent') })
            return
          }
          if (Buffer.byteLength(payload.content, 'utf8') > MAX_SOURCE_BYTES) {
            writeJson(res, 413, {
              error: T(lang, 'contentTooLarge', MAX_SOURCE_BYTES),
            })
            return
          }
          const workspace = resolveWorkspaceDir(payload.cwd, lang)
          if (workspace.error) {
            writeJson(res, 400, { error: workspace.error })
            return
          }
          try {
            ...
            if (typeof payload.target === 'string' && payload.target.trim() !== '') {
              const resolved = await resolveTargetPath(payload.target, workspace.dir, global.dshHome, lang)
              if (resolved.error) {
                writeJson(res, 403, { error: resolved.error })
                return
              }
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (typeof payload.globalEnabled !== 'boolean') {
            writeJson(res, 400, { error: T(lang, 'missingGlobalEnabled') })
            return
          }
          const workspace = resolveWorkspaceDir(payload.cwd, lang)
          if (workspace.error) {
          ...
          try {
            ...
            if (payload.globalEnabled === before.paused) {
              ...
              if (!payload.globalEnabled) {
                ...
                if (!source.exists) {
                  writeJson(res, 404, { error: T(lang, 'noGlobalToPause', before.activePath) })
                  return
                }
              }
              ...
              if (result.changed) {
                ctx.logger?.info?.(
                  payload.globalEnabled
                    ? T(lang, 'globalEnabled', result.path)
                    : T(lang, 'globalPaused', result.path),
                )
              }
            }
            const global = await resolveGlobalTarget()
            const budget = await buildBudget(await buildLayerView(workspace.dir, global), lang)
            writeJson(res, 200, {
              ok: true,
              activation: {
            ...
            writeJson(res, 500, { error: T(lang, 'switchFailed', error?.message ?? T(lang, 'unknown')) })
          }
        },
      }),
          ...
          try {
            ...
            if (typeof payload.mode === 'string') {
              if (['both', 'project', 'global'].indexOf(payload.mode) < 0) {
                writeJson(res, 400, { error: T(lang, 'badMode') })
                return
              }
              ...
              writeJson(res, 400, { error: T(lang, 'missingModeOrChanges') })
              return
            }
            if (changes.length > 64) {
              writeJson(res, 400, { error: T(lang, 'tooManyLayers') })
              return
            }
            ...
            for (const change of changes) {
              if (!change || typeof change.base !== 'string' || typeof change.enabled !== 'boolean') {
                writeJson(res, 400, { error: T(lang, 'badChangeItem') })
                return
              }
              const base = resolve(change.base)
              const hit = allowed.get(pathKey(base))
              if (!hit || hit.pausedVariant) {
                writeJson(res, 403, { error: T(lang, 'layerOutOfChain', base) })
                return
              }
            ...
            writeJson(res, 200, {
              ...
              layersView,
              mode: deriveMode(layersView),
              budget: await buildBudget(layersView, lang),
            })
          } catch (error) {
            writeJson(res, 500, { error: T(lang, 'switchFailed', error?.message ?? T(lang, 'unknown')) })
          }
